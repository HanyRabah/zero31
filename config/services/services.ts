import { Service } from "./types";

export const services: Record<string, Service> = {
	architecture: {
		id: "architecture",
		title: "Architecture",
		description:
			"Designing for Work - Some of the most established companies have trusted us with designing their spaces where they get work done. We think of a workspace as a second home; as an average person spends more time in the office per week than his own home. Our approach is always to focus on productivity, focus and creative inspirations; while first and foremost creating a healthy social and human at the workplace.",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
			{
				title: "Lighting for Hotels",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo.",
				image: "/images/projects/north-beach-house.png",
				imageAlt: "Hotel lighting solutions",
				backgroundColor: "bg-novo-blue",
				link: "/services/architecture#lighting",
				reverse: true,
			},
			{
				title: "Lighting for Hotels",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo.",
				image: "/images/projects/north-beach-house.png",
				imageAlt: "Hotel lighting solutions",
				backgroundColor: "bg-off-white",
				link: "/services/architecture#lighting",
			},
		],
	},
	"interior-design": {
		id: "interior-design",
		title: "Interior Design",
		description:
			"Designing for Work - Some of the most established companies have trusted us with designing their spaces where they get work done. We think of a workspace as a second home; as an average person spends more time in the office per week than his own home. Our approach is always to focus on productivity, focus and creative inspirations; while first and foremost creating a healthy social and human at the workplace.",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"orem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum. Nunc ac velit gravida, tristique ipsum pul",
				image: "/images/services/architecture-1.jpg",
				imageAlt: "Architecture design process",
				link: "/services/architecture#design",
			},
		],
	},
	"interior-architecture": {
		id: "interior-architecture",
		title: "Interior Architecture",
		description:
			"Designing for Work - Some of the most established companies have trusted us with designing their spaces where they get work done. We think of a workspace as a second home; as an average person spends more time in the office per week than his own home. Our approach is always to focus on productivity, focus and creative inspirations; while first and foremost creating a healthy social and human at the workplace.",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	"lighting-design": {
		id: "lighting-design",
		title: "Lighting Design",
		description:
			"Designing for Work - Some of the most established companies have trusted us with designing their spaces where they get work done. We think of a workspace as a second home; as an average person spends more time in the office per week than his own home. Our approach is always to focus on productivity, focus and creative inspirations; while first and foremost creating a healthy social and human at the workplace.",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	"custom-furniture": {
		id: "custom-furniture",
		title: "Custom Furniture",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	"ffae-service": {
		id: "ffae-service",
		title: "FF&E Service",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	landscape: {
		id: "landscape",
		title: "Landscape",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	"project-management": {
		id: "project-management",
		title: "Project Management",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	hospitality: {
		id: "hospitality",
		title: "Hospitality",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	residential: {
		id: "residential",
		title: "Residential",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	commercial: {
		id: "commercial",
		title: "Commercial",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
	"office-design": {
		id: "office-design",
		title: "Office Design",
		description: "asdf",
		sections: [
			{
				title: "Full turn-key solutions",
				subtitle: "Taste making and object curation that completes the experience of each project.",
				description:
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus congue sit amet felis a tincidunt. Fusce tincidunt dolor ac nulla commodo, eu laoreet tortor rutrum.",
				image: "/images/projects/new-giza-residence.png",
				imageAlt: "Full turn-key solutions",
				link: "/services/architecture#design",
				backgroundColor: "bg-white",
			},
		],
	},
};
